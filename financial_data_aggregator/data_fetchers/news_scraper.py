...
def fetch_alpha_vantage_news(ticker_symbol, api_key, time_from_dt=None, time_to_dt=None, sort='LATEST', limit=10):
    ...
    Args:
        ...
        limit (int): The number of news articles to return (default 10, max 1000 for premium plans).# news_scraper.py
        filename = f"{ticker_symbol}_NEWS_{week_number}.csv"
    ...
    try:
        ...
        logging.error(f"    Request failed for Alpha Vantage news for {ticker_symbol}: {e}")
        return None
    except ValueError as e: # Includes JSONDecodeError
        logging.error(f"    Failed to decode JSON for Alpha Vantage news for {ticker_symbol}: {e}")
        return None
    except Exception as e:
        logging.error(f"    An unexpected error occurred fetching Alpha Vantage news for {ticker_symbol}: {e}")
        return None

def fetch_and_save_news(ticker_symbol, week_start_date, week_end_date, api_key, output_dir, week_number, limit=10):
    """
    ...
    """
    logging.info(f"    Fetching Alpha Vantage news for {ticker_symbol} (Week {week_number}) for period {week_start_date.strftime('%Y-%m-%d')} to {week_end_date.strftime('%Y-%m-%d')}...")
    ...
    try:
        ...
        for item in all_news_items:
            ...
            try: # Keep try/except for parsing
                ...
                logging.warning(f"    Could not parse time_published: {time_published_str} for article: {item.get('title')}")
                continue
            ...
            if compare_week_start_utc <= publish_datetime_utc <= compare_week_end_utc:
                article_data = {
                    ...
                    'time_published_iso': publish_datetime_utc.isoformat(), # Kept for sorting, will be dropped before CSV
                    'authors': item.get('authors', []),
                    'summary': item.get('summary'),
                    'source': item.get('source'),
                    # Other fields like banner_image, source_domain, sentiment scores are now excluded
                }
        ...
        if not news_df.empty:
            # Define desired columns in the specific order for the CSV output
            desired_columns_for_csv = ['ticker', 'title', 'url', 'time_published_str', 'authors', 'summary', 'source']
            
            # Select and reorder columns for the CSV file
            news_df_to_save = news_df[desired_columns_for_csv]
            
            news_df_to_save.to_csv(filepath, index=False, encoding='utf-8')
            logging.info(f"    [✓] News saved to {filepath} ({len(news_df_to_save)} articles)")
