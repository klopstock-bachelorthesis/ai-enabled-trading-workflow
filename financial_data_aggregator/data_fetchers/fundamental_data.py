...
# Define the list of required columns for the final output
REQUIRED_FUNDAMENTAL_COLUMNS = [
    ...
    'PriceToBookRatio', 'EVToRevenue', 'EVToEBITDA', 'Beta', 'SharesOutstanding', 'DividendDate',
    'ExDividendDate', 'fiscalDateEnding', 'reportedCurrency', 'grossProfit', 'totalRevenue',
    ...
    'commonStock', 'commonStockSharesOutstanding', 'operatingCashflow', 'paymentsForOperatingActivities',
    ...
    'changeInExchangeRate', 'reportedEPS'
]
...
def get_overview_df(data):
    ...
    return pd.DataFrame() # Return empty if not a dict (and not None/empty)
...
def fetch_and_save_financials(ticker, api_key, output_dir, week_number):
    """
    ...
    """
    logging.info(f"    Fetching financial fundamentals for {ticker} (Week {week_number})...")
    ...
    # Reindex to ensure only required columns are present and in the specified order
    # Missing columns will be filled with NaN, extra columns will be dropped.
    final_df = final_df.reindex(columns=REQUIRED_FUNDAMENTAL_COLUMNS)

    # File naming convention
    filename = f"{ticker}_{week_number}.csv" # Adjusted to TICKER_WEEKNUMBER.csv
