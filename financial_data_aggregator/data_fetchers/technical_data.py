...
def fetch_daily_time_series(symbol, api_key, outputsize='compact'): # compact for last 100, full for full history
    ...
    try:
        ...
        if 'Time Series (Daily)' not in data:
            logging.warning(f"    No 'Time Series (Daily)' data found for {symbol}. API response: {data.get('Note') or data.get('Information') or data.get('Error Message')}")
            return None
        ...
        # Keeping 'close' as per current implementation
        ohlcv_df = df[['open', 'high', 'low', 'close', 'volume']].astype(float)
        
        return ohlcv_df
    except requests.exceptions.RequestException as e:
        logging.error(f"    Request failed for daily time series {symbol}: {e}")
        return None
    except ValueError as e: # Includes JSONDecodeError
        logging.error(f"    Failed to decode JSON for daily time series {symbol}: {e}")
    except Exception as e:
        logging.error(f"    An unexpected error occurred fetching daily time series for {symbol}: {e}")
        return None
...
def fetch_macd(symbol, api_key, interval='daily', series_type='close', fastperiod=12, slowperiod=26, signalperiod=9):
    ...
    try:
        ...
        if "Technical Analysis: MACD" not in data:
            logging.warning(f"    No 'Technical Analysis: MACD' data found for {symbol}. API response: {data.get('Note') or data.get('Information') or data.get('Error Message')}")
            return None
        ...
        logging.error(f"    Request failed for MACD {symbol}: {e}")
        return None
    except ValueError as e: # Includes JSONDecodeError
        logging.error(f"    Failed to decode JSON for MACD {symbol}: {e}")
        return None
    except Exception as e:
        logging.error(f"    An unexpected error occurred fetching MACD for {symbol}: {e}")
        return None
...
def fetch_rsi(symbol, api_key, interval='daily', time_period=14, series_type='close'):
    ...
    try:
        ...
        if "Technical Analysis: RSI" not in data:
            logging.warning(f"    No 'Technical Analysis: RSI' data found for {symbol}. API response: {data.get('Note') or data.get('Information') or data.get('Error Message')}")
            return None
        ...
        logging.error(f"    Request failed for RSI {symbol}: {e}")
        return None
    except ValueError as e: # Includes JSONDecodeError
        logging.error(f"    Failed to decode JSON for RSI {symbol}: {e}")
        return None
    except Exception as e:
        logging.error(f"    An unexpected error occurred fetching RSI for {symbol}: {e}")
        return None
...
def fetch_bbands(symbol, api_key, interval='daily', time_period=14, series_type='close', nbdevup=2, nbdevdn=2, matype=0):
    ...
    try:
        ...
        if "Technical Analysis: BBANDS" not in data:
            logging.warning(f"    No 'Technical Analysis: BBANDS' data found for {symbol}. API response: {data.get('Note') or data.get('Information') or data.get('Error Message')}")
            return None
        ...
        logging.error(f"    Request failed for BBANDS {symbol}: {e}")
        return None
    except ValueError as e: # Includes JSONDecodeError
        logging.error(f"    Failed to decode JSON for BBANDS {symbol}: {e}")
        return None
    except Exception as e:
        logging.error(f"    An unexpected error occurred fetching BBANDS for {symbol}: {e}")
        return None


def fetch_and_save_ohlcv(ticker, week_start_date, week_end_date, api_key, output_dir, week_number):
    """
    ...
    Args:
        ...
        week_number (int): The sequential number of the week being processed.
    """
    logging.info(f"    Fetching OHLCV, MACD, RSI & BBANDS for {ticker} (Week {week_number}) data relevant up to {week_end_date.strftime('%Y-%m-%d')}...")
    ...
    if daily_df is None or daily_df.empty: # Check after OHLCV fetch
        logging.warning(f"    No OHLCV data fetched for {ticker}. Skipping CSV save.")
        return None
    ...
    if macd_df is not None and not macd_df.empty:
        ...
        logging.info(f"    [✓] MACD data merged for {ticker}.")
    else:
        logging.warning(f"    No MACD data fetched for {ticker} or MACD data is empty. Proceeding with OHLCV only.")
    ...
    if rsi_df is not None and not rsi_df.empty:
        ...
        logging.info(f"    [✓] RSI data merged for {ticker}.")
    else:
        logging.warning(f"    No RSI data fetched for {ticker} or RSI data is empty.")
    ...
    if bbands_df is not None and not bbands_df.empty:
        ...
        logging.info(f"    [✓] BBANDS data merged for {ticker}.")
    else:
        logging.warning(f"    No BBANDS data fetched for {ticker} or BBANDS data is empty.")
    ...
    if relevant_period_df.empty: # Check after filtering
        logging.warning(f"    No data found for {ticker} on or before {week_end_date.strftime('%Y-%m-%d')} after merging. Skipping CSV save.")
        return None
    ...
    if last_200_days_df.empty:
        logging.warning(f"    Not enough data to extract 200 trading days for {ticker} ending by {week_end_date.strftime('%Y-%m-%d')}. Skipping CSV save.")
        return None

    # Update filename to be more descriptive
    filename = f"{ticker}_TA_{week_number}.csv"
    filepath = os.path.join(output_dir, filename)
    
    try:
        last_200_days_df.to_csv(filepath, index=True) # Keep date as index
        logging.info(f"    [✓] Technical data saved to {filepath} ({len(last_200_days_df)} rows)")
        return filepath # Return filepath on successful save
    except Exception as e:
        logging.error(f"    Error saving technical data for {ticker} to {filepath}: {e}")
        return None