...
portfolio = load_csv("~/Downloads/bachelorarbeit/results_workflow.csv")
prices = load_csv("~/Downloads/bachelorarbeit/NVDA_MSFT_AAPL_prices.csv", parse_dates=["date"])
benchmark = load_csv("~/Downloads/bachelorarbeit/benchmark_prices_S&P.csv", parse_dates=["date"])
macd_daily_portfolio = load_csv("~/Downloads/bachelorarbeit/macd_daily_strategy_weights.csv")
...
(vol_df['Portfolio Return'].rolling(window).std() * np.sqrt(52)).plot(label='Portfolio Volatility', color=red, linewidth=1.5)
(vol_df['Benchmark Return'].rolling(window).std() * np.sqrt(52)).plot(label='S&P 500 Volatility', color=blue, linewidth=1.5, linestyle='--')
(vol_df['B&H Equal-Weight Return'].rolling(window).std() * np.sqrt(52)).plot(label='B&H Equal-Weight Volatility', color=orange, linewidth=1.5, linestyle=':')
(vol_df['B&H NVDA Return'].rolling(window).std() * np.sqrt(52)).plot(label='B&H NVDA Volatility', color=green, linewidth=1.5, linestyle=':')
(vol_df['MACD Daily Return'].rolling(window).std() * np.sqrt(52)).plot(label='MACD Daily Volatility', color=purple, linewidth=1.5, linestyle='-.')

plt.title(f'{window}-Week Rolling Annualized Volatility')
...
plt.plot(merged_compare["Effective_Week"], merged_compare["Cumulative Benchmark Return"],
         label="S&P 500", linewidth=1.5, marker='s', markersize=2, color=blue)
...
merged_compare["SP500 Drawdown"] = calculate_drawdown_pct(merged_compare["Cumulative Benchmark Return"])
...
# S&P 500 drawdown
plt.fill_between(merged_compare["Effective_Week"], 
                 merged_compare["SP500 Drawdown"], 
                 color='blue', alpha=0.2, label='S&P 500 Drawdown')
# B&H NVDA drawdown
plt.fill_between(merged_compare["Effective_Week"],
...
metrics_data = {
    ...
    'S&P 500': {
        ...
        'Max Drawdown': merged_compare['SP500 Drawdown'].min(),
    },
     'B&H (Equal Weight)': {
...
benchmark_returns = merged_compare[return_cols['S&P 500']].fillna(0)
...
print(f"Correlation of weekly returns with S&P 500: {correlation:.2f}")
...
plt.xlabel("S&P 500 Weekly Return")
